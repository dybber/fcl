...
check _ _ (IntE v)    = (IntT, IntE v)
check _ _ (DoubleE v) = (DoubleT, DoubleE v)
check _ _ (BoolE v)   = (BoolT, BoolE v)
check _ _ UnitE       = (UnitT, UnitE)
...
check senv tenv (ReduceSeqE f e arr NoType) =
  ...
  in case (ft, arrt) of
       ((at :*: bt) :> at', ArrayT bt' _) -> if at == at' && at == et && bt == bt'
