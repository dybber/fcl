...
module FCL.TypeInference (typeinferCore, TypeErrorCore(..)) where
...
data TypeErrorCore = UnificationError SourceRegion Type Type
...
type TI x = StateT TVE (Except TypeErrorCore) x

throwError :: TypeErrorCore -> TI a
throwError err = lift (throwE err)

runTI :: TI a -> TVE -> Either TypeErrorCore (a, TVE)
...
infer env (App e1 e2) = do
  ...
  (t2,e2') <- infer env e2
  tv <- newtv
  unify (newRegion Missing Missing) t1 (t2 :> tv)
  return (tv, App e1' e2')
...
infer env (AppLvl e lvl) = do
  ...
  (te,e') <- infer env e
  tv <- newtv
  unify (newRegion Missing Missing) te (lvlvar :-> tv)
...
typeinferCore :: [Definition a] -> Either TypeErrorCore [Definition Type]
typeinferCore prog =
...
typecheckProg tenv (d : ds) = do
  ...
  case defSignature d of
    Just sig -> unify (newRegion Missing Missing) sig ty
    Nothing -> return ()
